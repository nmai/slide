...
public_functions.addObject = function( description ) {
	...
	*/
	...
	body = new CANNON.Body({ mass: description.mass, shape: shape });
...
//TODO: Needs to be tweaked in order to get chunks to actually fucking work.
//		Right now, only the first chunk is sent but the number of objects reported can be higher.
reportWorldExperimental = function( chunknum ) {
	var index, object,
		report = [], i = 0, offset = 0,
		origin, rotation, maxreported; 

	//if (typeof chunknum === 'undefined') chunknum = 1;

	if (_num_objects / (chunknum * REPORT_CHUNKSIZE) < 1) 
		maxreported = _num_objects % REPORT_CHUNKSIZE;
	else maxreported = REPORT_CHUNKSIZE;
	...
	worldreport[1] = maxreported; // record how many objects we're reporting on
	
	for ( var index = (chunknum-1)*REPORT_CHUNKSIZE; index < (chunknum-1)*REPORT_CHUNKSIZE+maxreported; index++ ) {
		if ( _objects.hasOwnProperty( _objects[index] ) ) {
			object = _objects[index];
	...
	if((chunknum+1)*REPORT_CHUNKSIZE <= _num_objects)
		reportWorld(chunknum+1);
};
