...
var Physijs = (function() {
	...
	//Physijs.RaycastVehicle
	Physijs.RaycastVehicle = function ( param1, param2 ) {
		Eventable.call( this );

		//Define a bunch of shit here, do whatever setup

		//Avoid accessing anything related to the world.
		//Should be able to create meshes here
	};
	Physijs.RaycastVehicle.prototype.constructor = Physijs.RaycastVehicle;
	Eventable.make( Physijs.RaycastVehicle );

	//Basically the rest of the available functions are going to be proxies to setter methods
	//I *will* need to replace the standard addToWorld func with a stripped-down version that
	//only adds the callbacks.

	//Figure out how to use the events to get vehicle data, or just use reports.
	
	/*
	// Physijs.RaycastVehicle.mass
	Physijs.RaycastVehicle.prototype.__defineGetter__('mass', function() {
		return this._physijs.mass;
	});
	Physijs.RaycastVehicle.prototype.__defineSetter__('mass', function( mass ) {
		this._physijs.mass = mass;
		if ( this.world ) {
			this.world.execute( 'updateMass', { id: this._physijs.id, mass: mass } );
		}
